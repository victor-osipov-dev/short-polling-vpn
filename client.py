...
class ClientTunnel:
    ...
    def _available_urls(self):
        ...
        if not avail and self.server_urls:
            logger.warning("[client] all server URLs quarantined; resetting cooldowns")
    ...
    def _mark_success(self, url):
        if url in self._url_failures or url in self._url_cooldown_until:
            ...
            logger.info(f"[client] URL recovered: {url}")
    ...
    def _mark_failure(self, url):
        ...
        if fails >= self.url_fail_limit and url not in self._url_cooldown_until:
            ...
            logger.warning(f"[client] URL quarantined for {self.url_cooldown_seconds}s: {url}")
    ...
    async def _poll_once(self):
        ...
        try:
            ...
            async with self.http.stream(self.poll_method, url_base + self.poll_path, **kwargs) as resp:
